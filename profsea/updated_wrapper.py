...
def simulation_task(random_idx: int, iteration_number: int) -> dict:
    """Runs the global emulator components for a single ensemble member across scenarios."""
    ...
    for idx, scenario in enumerate(worker_scenarios):
        # The new API requires reinstantiating components or updating their state per run.
        # ThermalExpansion requires OHC_change in the constructor.
        slr_components = {
            "expansion": ThermalExpansion(
                OHC_change=np.expand_dims(ohc_sampled[idx, :], axis=0)
            ),
            "greenland": GreenlandAR6(),
            "landwater": LandwaterAR6(),
            "wais": AntarcticaISMIP6(params_path="/Users/gregorymunday/Documents/Papers/ProFSea/ProFSea-tool/profsea/components/aux_data/wais_params.nc"),
            "eais": AntarcticaISMIP6(params_path="/Users/gregorymunday/Documents/Papers/ProFSea/ProFSea-tool/profsea/components/aux_data/eais_params.nc"),
            "pen": AntarcticaISMIP6(params_path="/Users/gregorymunday/Documents/Papers/ProFSea/ProFSea-tool/profsea/components/aux_data/aispen_params.nc"),
            "glacier": Glacier()
        }
        ...
        projections = global_model.run(
            scenario=scenario,
            T_change=np.expand_dims(tas_sampled[idx, :], axis=0),
            member_seed=iteration_number,
        )

        # Print the shame of all components
        for comp_name, comp_data in projections.items():
            console.log(f"Scenario: {scenario}, Component: {comp_name}, Shape: {comp_data.shape}")

        global_model.sum_components(projections)

        # Aggregate dyn/smb components to match original data structures 
        results[scenario]["gmslr"] = global_model.results["gmslr"]
        results[scenario]["expansion"] = projections["expansion"]
        results[scenario]["antarctica"] = projections["wais"] + projections["eais"] + projections["pen"]
        results[scenario]["greenland"] = projections["greenland"]
        results[scenario]["glacier"] = projections["glacier"]
        results[scenario]["landwater"] = projections["landwater"]
...
def main(args):
    ...
    # ) as exectutor:
    #     futures = [exectutor.submit(simulation_task, idx, i) for i, idx in enumerate(random_indices)]
    ...
    # sampled_tas = np.asarray(sampled_tas) 
    ...
    # 1. Get your 1000 random climate members
    n_iterations = 1000
    rng = np.random.default_rng()
    random_indices = rng.integers(0, high=tas.shape[1], size=n_iterations)
    ...
    # 2. Loop only over scenarios (e.g., SSPs)
    for idx, scenario in enumerate(scenarios):
        console.log(f"Projecting {scenario}...")
        
        # Sliced matrices: shape (1000, 295)
        ...
        slr_components = {
            ...
            "greenland": GreenlandAR6(),
            "landwater": LandwaterAR6(),
            "wais": AntarcticaISMIP6(params_path="/Users/gregorymunday/Documents/Papers/ProFSea/ProFSea-tool/profsea/components/aux_data/wais_params_APR_nodrift.nc"),
            "eais": AntarcticaISMIP6(params_path="/Users/gregorymunday/Documents/Papers/ProFSea/ProFSea-tool/profsea/components/aux_data/eais_params_APR_nodrift.nc"),
            "pen": AntarcticaISMIP6(params_path="/Users/gregorymunday/Documents/Papers/ProFSea/ProFSea-tool/profsea/components/aux_data/pen_params_APR_nodrift.nc"),
            "glacier": Glacier()
        }
        ...
        projections["antarctica"] = projections.pop("wais") + projections.pop("eais") + projections.pop("pen")
